...
FOR EACH ROW
BEGIN
    IF CHECK_VALUE_EXIST(:NEW.role, 'role', 'CYPI_CERGY.ROLES_UTILISATEURS') THEN
...
FOR EACH ROW
BEGIN
    IF CHECK_VALUE_EXIST(:NEW.groupe, 'groupe', 'CYPI_CERGY.GROUPES_UTILISATEURS') THEN
...
FOR EACH ROW
BEGIN
    IF CHECK_VALUE_EXIST(:NEW.emplacement, 'emplacement', 'CYPI_CERGY.EMPLACEMENTS') THEN
...
FOR EACH ROW
BEGIN
    IF CHECK_VALUE_EXIST(:NEW.nom, 'nom', 'CYPI_CERGY.MATERIELS') THEN
...
FOR EACH ROW
BEGIN
    IF CHECK_VALUE_EXIST(:NEW.email, 'email', 'CYPI_CERGY.UTILISATEURS') THEN
        RAISE_APPLICATION_ERROR(-20009, 'Email ' || :NEW.email || ' existe déjà.');
    END IF;
 
    IF :NEW.fk_emplacement IS NOT NULL AND NOT CHECK_VALUE_EXIST(:NEW.fk_emplacement, 'id_emplacement', 'CYPI_CERGY.EMPLACEMENTS') THEN
...
FOR EACH ROW
BEGIN
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_createur, 'id_utilisateur', 'CYPI_CERGY.UTILISATEURS') THEN
        RAISE_APPLICATION_ERROR(-20011, 'Utilisateur ID ' || :NEW.fk_createur || ' n''existe pas.');
    END IF;
 
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_priorite, 'id_priorite', 'CYPI_CERGY.PRIORITES_TICKETS') THEN
        RAISE_APPLICATION_ERROR(-20013, 'Priorité ID ' || :NEW.fk_priorite || ' n''existe pas.');
    END IF;
 
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_emplacement, 'id_emplacement', 'CYPI_CERGY.EMPLACEMENTS') THEN
...
CREATE OR REPLACE TRIGGER CYPI_CERGY.trg_upsert_commentaires_tickets
BEFORE INSERT ON CYPI_CERGY.COMMENTAIRES_TICKETS
FOR EACH ROW
BEGIN
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_ticket, 'id_ticket', 'CYPI_CERGY.TICKETS') THEN
        RAISE_APPLICATION_ERROR(-20019, 'Ticket ID ' || :NEW.fk_ticket || ' n''existe pas.');
    END IF;
 
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_utilisateur, 'id_utilisateur', 'CYPI_CERGY.UTILISATEURS') THEN
...
CREATE OR REPLACE TRIGGER CYPI_CERGY.trg_upsert_observateurs_tickets
BEFORE INSERT ON CYPI_CERGY.OBSERVATEURS_TICKETS
FOR EACH ROW
BEGIN
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_ticket, 'id_ticket', 'CYPI_CERGY.TICKETS') THEN
...
CREATE OR REPLACE TRIGGER CYPI_CERGY.trg_upsert_attributions_tickets
BEFORE INSERT ON CYPI_CERGY.ATTRIBUTIONS_TICKETS
FOR EACH ROW
BEGIN
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_ticket, 'id_ticket', 'CYPI_CERGY.TICKETS') THEN
...
CREATE OR REPLACE TRIGGER CYPI_CERGY.trg_upsert_tickets_ressources
BEFORE INSERT ON CYPI_CERGY.TICKETS_RESSOURCES
FOR EACH ROW
BEGIN
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_ticket, 'id_ticket', 'CYPI_CERGY.TICKETS') THEN
    ...
    IF NOT CHECK_VALUE_EXIST(:NEW.fk_ressource, 'id_ressource', 'CYPI_CERGY.RESSOURCES') THEN
